...
$query = mysql_query("select * from fd_perm_need WHERE active ='Y' ORDER BY id DESC ");  // change table name for duplicate and change order by from date to id 
		   ...
		   else 
		  {
			...
			$i++;					
								...
								<td id="replies_cell"><form action="replies_perm.php" method="POST"> <input type="hidden" id="job_id" name="job_name" value="<?php echo $row['id'];?>"> <button id="replies_button" type="submit"> 
									...
									$result=mysql_query("SELECT count(*) as total from fd_req_perm WHERE need_id = '$id_req' ");
									...
									$result_check_new=mysql_query("SELECT count(*) as total from fd_req_perm WHERE need_id = '$id_req' and status ='new' ");
