...
$id = $_POST['job_name'];
   ...
   echo"<div id=\"need_style\">";
	$query = mysql_query("select * from dds_perm_need WHERE id = '$id'"); // change table name for duplicate
	...
	$query = mysql_query("select * from dds_req_perm WHERE need_id = '$id' ORDER BY time DESC"); // change table name for duplicate
	while($row = mysql_fetch_assoc($query)){ 
