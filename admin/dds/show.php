	...
	while($row = mysql_fetch_assoc($query)){ 
		   ...
		   else 
		  {
			...
			$i++;					
								...
								<td id="replies_cell"><form action="replies.php" method="POST"> <input type="hidden" id="job_id" name="job_name" value="<?php echo $row['id'];?>"> <button id="replies_button" type="submit"> 
									<?php
									
									$id_req = $row['id'];
									
									$result=mysql_query("SELECT count(*) as total from dds_req_temp WHERE need_id = '$id_req' "); //change this VERSION 6
									...
									$result_check_new=mysql_query("SELECT count(*) as total from dds_req_temp WHERE need_id = '$id_req' and status ='new' "); //change this VERSION 6
