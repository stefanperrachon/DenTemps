...
$id = 67;
	...
	//echo "<div>test</div>". $id."<div>test 2</div>".$id."<div>test 3</div>";
	

$query = mysql_query("select * from daii_temp_need WHERE active='Y' ORDER BY date "); 
	...
	while($row = mysql_fetch_assoc($query)){ 
       ...
       <!--copy this-->		   <td id="replies_cell"><form action="replies.php" method="POST"> <input type="hidden" id="job_id" name="job_name" value="<?php echo $row['id'];?>"> <button id="replies_button" type="submit"> 
									...
									$result=mysql_query("SELECT count(*) as total from daii_req_temp WHERE need_id = '$id_req' "); //change this VERSION 6
									...
									$result_check_new=mysql_query("SELECT count(*) as total from daii_req_temp WHERE need_id = '$id_req' and status ='new' "); //change this VERSION 6
	...
	<!--change value to archive--><td id="button_cell"><form action="index.php" method="POST" onsubmit="return confirm('Are you sure??');" class="delete_form"> <input type="hidden" id ="id_num" name="id_name" value="<?php echo $row['id'];?>"> <input id="delete_button" type="submit" value="archive"></form></td>
								
 <!--copy this and replace from tr - body -->	
	<?php } ?>
	</tr>
								</table>
	</div>
