...
<body>
<div id="go_back_link">
<h1><a  href="perm_form.php"><--Go back to Needs</a></h1>
</div>
<div id="replies_dec">
<div id="data_style">
<?php
...
$id = $_POST['job_name'];
   ...
   mysql_query("
   UPDATE daii_req_perm
   SET status='seen'
   WHERE need_id = '$id'");

   echo"<div id=\"need_style\">";
	$query = mysql_query("select * from daii_perm_need WHERE id = '$id'"); // change table name for duplicate
	while($row = mysql_fetch_assoc($query)){ 
	
	echo "<div id=\"office_name\">";
	echo $row['name']."</div><div id=\"need_date\"> ".$row['duration'];
	echo"</div>";
	...
	$query = mysql_query("select * from daii_req_perm WHERE need_id = '$id' ORDER BY time DESC"); // change table name for duplicate
	while($row = mysql_fetch_assoc($query)){ 
