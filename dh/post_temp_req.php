...
mysql_query("INSERT INTO dh_req_temp VALUES ('', '$temp_name', '$email', '$phone ', '$notes', '$need_id', DATE_ADD(NOW(), INTERVAL 1 HOUR),'$ipaddress','new' ) "); 

	$query = mysql_query("select * from dh_temp_need WHERE id = '$need_id'");  // change table name for duplicate
	while($row = mysql_fetch_assoc($query)){ 
	
	$office = $row['office_location'];
	$date = date('m/d/Y', strtotime($row['date'])); 
	$name = $row['name']; 
	$onotes = $row['notes']; // change this for v.3
	$id = $row['id']; 
}

		require '../PHPMailer-master/PHPMailerAutoload.php';  // changed this for v.3 

//mail to temp

$temp_subject = "Your request about ".$date." in ".$office;
$temp_msg = "Hello ".$temp_name.", \n \n You have requested more information for the temporary assignment on ".$date." in ".$office.". \n We will contact you back if the assignment is still available. \n Feel free to call us with any questions/concerns at 919-461-9655. \n \n Thank you, \n DenTemps Staff";
...
$temp_headers = "From: ".$DT_email;
							...
							$body_temp = file_get_contents('contents_temp_temp.html');
							$body_temp = str_replace('$temp_name', $temp_name, $body_temp);
							$body_temp = str_replace('$date', $date, $body_temp);
...
// mail to dt
$DT_sub = $temp_name.", DH requests ".$date." in ".$office;
$DT_msg = $temp_name." has requested more information about the assignment on ".$date." in ".$office.". \n The notes for the assignment are: \n".$onotes."\n \n The id of the assignment is ".$id." \n \n The temp filled out the info form as follows: \n Name: ".$temp_name."\n Phone: ".$phone."\n Email: ".$email."\n Temps comments/questions: \n ".$notes." \n"; 
$DT_headers = "From: ".$email;
								...
								//Read an HTML message body from an external file, convert referenced images to embedded,
								//convert HTML into a basic plain-text alternative body
								$body = file_get_contents('contents_admin_temp.html');
